...
const BAKED_OFFICIAL = {
  ...
  minimumGaranti: { value: 4.35, date: "2026-06-01" },   // MG — revalorisé avec le SMIC (avantage nourriture HCR : 1 MG/repas ; source service-public/URSSAF)
...
const RATES_2026_AGRI = Object.assign({}, RATES_AGRI, {
  ...
  autresPat: 0.30 + 0.50 + 0.016 + 1.00 + 0.30 + 0.68 + 0.45 + 0.42
  // référence ENTITÉ ≥ 50 salariés (effectif sécurité sociale, intérimaires compris — BOSS Effectif §310-320, R. 130-1 II CSS) :
  // CSA 0,30 + FNAL 0,50 + dialogue social 0,016 + CFP 1,00 (L. 6331-3) + conventionnelle formation TT 0,30 (L. 6331-69, Akto) + apprentissage 0,68 + PEEC 0,45 + SST MSA 0,42
  // La bande d'effectif choisie (i.effectif) retranche FNAL 0,40 / PEEC 0,45 sous 50 et CFP 0,45 sous 11 ; le versement mobilité (i.vmPct) s'ajoute.
});
...
const RATES_2026_TARIFAIRE = Object.assign({}, RATES_TARIFAIRE, {
  ...
  mutuellePat: 0.0874,
  autresPat: 0.30 + 0.50 + 0.016 + 1.00 + 0.30 + 0.68 + 0.45
  // référence ENTITÉ ≥ 50 salariés (effectif sécurité sociale, intérimaires compris — BOSS Effectif §310-320, R. 130-1 II CSS) :
  // CSA 0,30 + FNAL 0,50 + dialogue social 0,016 + CFP 1,00 (L. 6331-3) + conventionnelle formation TT 0,30 (L. 6331-69, Akto) + apprentissage 0,68 + PEEC 0,45 = 3,246
  // La bande d'effectif choisie (i.effectif) retranche FNAL 0,40 / PEEC 0,45 sous 50 et CFP 0,45 sous 11 ; le versement mobilité (i.vmPct) s'ajoute (classeur : 3,766)
});
...
function defaultInputs(DB, sector) {
  ...
  const common = {
    ...
    participationHorsPaie: false, // true = participation logement par PRÉLÈVEMENT BANCAIRE : net de paie non réduit, coût logement toujours amorti
                                  // (le sélecteur de cible du pilotage et ce mode sont SYNCHRONISÉS : AVEC PARTICIPATION = retenue, SANS = prélèvement)
    agence: "",                 // agence PALMA sélectionnée (fixe le taux AT par défaut ; portée dans l'étude de prix)
    /* effectif : bande d'effectif « sécurité sociale » de l'entité, INTÉRIMAIRES COMPRIS ("50plus" | "20-49" | "11-19" | "moins11"),
       fixée par l'interface (défaut 50 et plus) ; absente ici pour que les vecteurs de test du classeur restent ceux du classeur */
    vmPct: 0,                   // versement mobilité, % du brut, taux de la zone où l'intérimaire travaille (D. 2333-91 CGCT) ; 0 = hors zone
    ...
    /* leviers d'optimisation (décochés = pratique maison actuelle) — cf. bulletin ACTERIM 07/2026 :
       cetRepos = IFM/ICCP placés au CET et restitués en HEURES DE REPOS payées au taux de base (ces heures
       entrent dans le SMIC de référence → la réduction générale s'applique au différé au lieu des 45 % provisionnés) ;
       hsRepos = la majoration des heures sup (argent sans heure) est placée au CET et restituée en heures. */
    cetRepos: false,
    hsRepos: false,
    ...
    contrat: "ctt",             // "ctt" | "saisonnier" (mêmes coûts, atouts opérationnels) | "cdii" | "cdie"
    tauxOccupation: 100,        // % heures en mission / heures payées (CDI-I/CDI-E : provision d'intermission)
    offreExpr: "coef",          // expression de l'offre dans l'étude : "coef" (coefficient) ou "taux" (taux global facturé) — affichage seul
...
function computeSheet(i, variant, hiddenAvance, f77Override) {
  ...
  /* CONTRATS (recherche vérifiée 01/09/2026, sources officielles + KALI authentique) :
   * "ctt" (défaut) : régime standard.
   * "saisonnier" : modélisé AVEC accord d'entreprise écartant l'IFM (L.1251-33 : un accord
   *   collectif étendu de la branche TT OU un accord d'entreprise/établissement peut écarter
   *   l'IFM pour les emplois à caractère saisonnier — décision direction 01/09/2026 : régler
   *   le simulateur comme si l'accord était conclu, pour DÉMONTRER le gain de marge et le
   *   faire signer). ICCP CONSERVÉE (L.1251-19, toujours due) — ce qui maintient la majoration
   *   RGDU ×1,1 (D.241-10 CSS : conditionnée à l'ICCP, pas à l'IFM). Sans accord signé, les
   *   coûts réels restent ceux du CTT standard.
   * "cdii" (CDI intérimaire, L.1251-58-1 s.) : ni IFM ni ICCP (L.1251-58-4) → provision CP de
   *   droit commun 1/10 ; RGDU SANS ×1,1 (D.241-10 CSS : b=1,1 réservé aux salariés dus ICCP).
   * "cdie" (CDI temps partagé aux fins d'employabilité, art. 115 loi 2018-771 relancé par la
   *   loi 2024-1027 — conclusions possibles jusqu'à ~15/11/2028) : idem cdii + AGS taux plein
   *   0,25 % (vs 0,03 % ETT) + PAS de frais de santé de branche + CPF 500 €/an. */
  const modeCDI = i.contrat === "cdii" || i.contrat === "cdie";
  const R0 = (i.contrat === "cdie")
    ? Object.assign({}, i.rates, {
        chomagePat: (i.rates.chomagePat || 0) + 0.22,
        mutuelleSal: 0, mutuellePat: 0 })
    : i.rates;
  /* Effectif « sécurité sociale » de l'entité (intérimaires compris — R. 130-1 II CSS, BOSS Effectif §310-320) : les taux de
   * référence sont ceux d'une entité ≥ 50 ; sous 50 : FNAL 0,10 au lieu de 0,50 et pas de PEEC ; sous 20 : déduction
   * forfaitaire HS 1,50 €/h au lieu de 0,50 (L. 241-18 / L. 241-18-1, plafond 250 supprimé par la LFSS 2026) ; sous 11 :
   * CFP 0,55 au lieu de 1 (L. 6331-1). Tdelta de la RGDU suit (0,3821 ≥ 50 / 0,3781 < 50). Versement mobilité ajouté au %.
   * Sans champ effectif (vecteurs de test du classeur), rien ne change. */
  let R = R0;
  if (i.effectif) {
    const b = i.effectif, petit50 = b !== "50plus", petit20 = b === "11-19" || b === "moins11", petit11 = b === "moins11";
    const ajust = (petit50 ? -0.40 - 0.45 : 0) + (petit11 ? -0.45 : 0) + (i.vmPct || 0);
    R = Object.assign({}, R0, { autresPat: (R0.autresPat || 0) + ajust, redPatHSupParH: petit20 ? -1.5 : -0.5 });
    i = Object.assign({}, i, { rgdu: Object.assign({}, i.rgdu || {}, { fnal050: !petit50 }) });
  } else if (i.vmPct) R = Object.assign({}, R0, { autresPat: (R0.autresPat || 0) + i.vmPct });
  const S = {};
  ...
  if (modeCDI && isMain) {
    /* CDI-I / CDI-E : ni IFM ni ICCP — provision CONGÉS PAYÉS de droit commun (1/10 du brut,
       L.3141-24), lissée en coût hebdo chargé sur la même assiette (versée à la prise des CP).
       Le garde (F32+F33)>0 -> D85=0 éteint de lui-même la provision CET (sans objet en CDI). */
    S.F32 = 0;
    S.D33 = S.D32;
    S.F33 = S.D33 * CONST.ICCP_TAUX;
  } else {
    /* saisonnier : IFM écartée par accord d'entreprise (L.1251-33) dans TOUTES les
       variantes (main, cachée, scénario IFM+ICCP) ; l'ICCP reste due — sans cascade
       puisque l'IFM n'existe plus dans l'assiette */
    const ifmWaived = i.contrat === "saisonnier";
    const ifmOn = ifmWaived ? false : (isMain ? (cfg.ifmMainToggle ? i.ifm : false) : true);  // caché : F11='OUI'
    S.F32 = ifmOn ? S.D32 * CONST.IFM_TAUX : 0;
    S.D33 = (cfg.iccpMode === "ifm") ? S.D32 + S.F32 : rowsSum;
    const iccpOn = isMain ? i.iccp : true;          // caché : F12='OUI' (inchangé — l'ICCP reste due en saisonnier)
  ...
  /* DFS : i.dfsFactor (ex. 0,93 = DFS BTP 7 % en 2026, sortie progressive BOSS) prime sur le facteur du secteur (classeur : 0,9) */
  S.D101 = S.F42 * ((i.dfsFactor != null && isFinite(+i.dfsFactor)) ? +i.dfsFactor : cfg.dfsFactor);
  ...
  /* Leviers « heures de repos CET » : le différé (D85, calculé ici par anticipation) et/ou la majoration des
   * heures sup sont restitués en heures payées au taux de base → ils comptent dans le SMIC de référence.
  ...
  S.heuresRG = S.D31 + S.hReposCet + S.hReposHS;
  {
    const rg = i.rgdu || {};
    const smicParam = (rg.smicRef || RGDU_DEFAULTS.smicRef) * S.heuresRG;
    ...
    let coeff = 0;
    if (S.remRG > 0 && S.remRG < plaf * smicParam) {
      const t = Math.min(1, 0.5 * (plaf * smicParam / S.remRG - 1));
      coeff = tmin + tdelta * Math.pow(t, p);
      coeff = Math.round(Math.min(coeff, tmin + tdelta) * 10000) / 10000;
    }
    S.rgduCoeff = coeff;
    S.rgduAmount = coeff * S.remRG * ((rg.interim === false || modeCDI) ? 1 : RGDU_DEFAULTS.interimMajoration);
  }
  ...
  if (cfg.exo === "tode") {
    ...
    if (S.levierCet && S.F42 > 0) S.rgduPanier *= S.remRG / S.F42;   /* les cotisations dues croissent avec le différé */
    S.H83 = (i.exoFormule === "rgdu") ? Math.min(S.rgduAmount, S.rgduPanier) : S.H83_fillon;
  }

  /* --- Provision CET (ligne 85) --- */
  /* provision CET : 21 % (IFM 10 % + ICCP 11 % en cascade) — saisonnier sous accord :
  ...
  /* Lecture défavorable du rattachement : le différé CET monétisé est réintégré dans
   * la rémunération de la DERNIÈRE MISSION RÉELLE (SMIC de référence inchangé) → le
   * coefficient chute et rogne le gain du stockage. Défaut maison = gain préservé
  ...
  if (S.D85 > 0 && i.exoFormule === "rgdu" && cfg.exo !== "tode" && !modeCDI && S.rgduAmount > 0 && S.F42 > 0) {
    ...
    const p = rg.p || RGDU_DEFAULTS.p;
    const plaf = rg.plafondSmic || RGDU_DEFAULTS.plafondSmic;
    const remuPire = S.F42 + S.D85;
    let coeffPire = 0;
    if (remuPire < plaf * smicParam) {
      const t = Math.min(1, 0.5 * (plaf * smicParam / remuPire - 1));
      coeffPire = tmin + tdelta * Math.pow(t, p);
      coeffPire = Math.round(Math.min(coeffPire, tmin + tdelta) * 10000) / 10000;
    }
  ...
  if (i.cetCoutsReels && S.D85 > 0) {
    ...
    S.cetCauserieCout = S.levierCet ? 0 : ((i.cetDebloc || 0) * (i.cetCauserieH || 0) / 52) * i.thBrut * 1.45;
  }
  ...
  /* CDI-I / CDI-E : provision d'INTERMISSION lissée sur les heures facturées —
   * (1/τ − 1) × coût salarial chargé de la semaine type (τ = taux d'occupation).
   * Approximation MAJORANTE : en intermission la garantie est la GMMR (CDI-I,
   * plancher SMIC×151,67 — L.1251-58-3) ou le dernier taux de base (CDI-E), sans
   * IFM/HS/indemnités. CDI-E : + abondement CPF 500 €/an (~9,62 €/sem, art. 115). */
  S.cdiIntermCout = 0; S.cdieFormationCout = 0;
  if (modeCDI) {
    const tau = Math.min(100, Math.max(1, i.tauxOccupation == null ? 100 : i.tauxOccupation)) / 100;
    S.cdiIntermCout = (1 / tau - 1) * (S.F42 + S.H88);
    if (i.contrat === "cdie") S.cdieFormationCout = 500 / 52;
  }
  S.O61 = S.O60 - S.F93 - S.recrutCout - (S.cetCoutReel || 0) - S.cdiIntermCout - S.cdieFormationCout;
  S.P61 = S.P60 - S.F93 - S.recrutCout - (S.cetCoutReel || 0) - S.cdiIntermCout - S.cdieFormationCout;
